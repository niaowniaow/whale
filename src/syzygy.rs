...
pub const TB_MAX_PIECES: usize = 5;
...
/// scale. Only unconditional results produce a bound.
pub fn probe_bound(board: &BoardState, ply: u8) -> Option<i16> {
    ...
    let bound = (TB_WIN - ply as i16).max(0);
    match loaded.tables.probe_wdl(&pos).ok()? {
        AmbiguousWdl::Win => Some(bound),
        AmbiguousWdl::Loss => Some(-bound),
        _ => None,
    }
}
